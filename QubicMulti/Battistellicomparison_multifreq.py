...
color = ['g', 'r', 'c', 'm', 'y', 'b', 'k']
color = color[0: len(NFREQS)]

delta_Dls_tot = []
delta_Dls_xpol_tot = []
for NF, c in zip(NFREQS, color):
    q = MultiQubicInstrument(
        NFREQS=NF, NPOINTS=NPOINTS, detector_nep=detector_nep) 

    # Battistelli
    f_sky, omega, apodization_factor = parameters(
        q, sampling, scene, maxiter, who='kaplan')
    ...
    xpols = np.load(
        ...
        'n{}_N{}_bin{}_{}MAPS_s{}_mi{}.npy'.format(NF, NPOINTS, 
...
mp.xticks(fontsize='x-large')
mp.yticks(fontsize='x-large')
mp.xlabel('$n$', fontsize='xx-large')
mp.ylabel(r'$\frac{{< \Delta D_\ell^{BB} (n)\ - \Delta D_\ell^{BB} (1)\ >}_\ell \ }{\Delta D_\ell^{BB} (1)\ } $', fontsize='xx-large')
mp.plot(NFREQS, mean_diff_monte, '-o',label='Monte Carlo')
mp.plot(NFREQS, mean_diff_theo, '-o',label='Theoretical')
mp.legend(loc='best')
