...
from pyoperators import (
    asoperator, BlockColumnOperator, IdentityOperator, ReciprocalOperator)
...
class MultiQubicScene(SceneHealpix):
    def __init__(self, nside=256, kind='IQU', absolute=False,
        ...
        absolute : boolean, optional
            ...
            the fluctuations in units of Kelvin, otherwise it only represents
            the fluctuations, in microKelvin.
        temperature : boolean, optional
            If true, the scene represents a temperature (absolute in Kelvin 
            ordifferential in microKelvin according the the absolute keyword)
            and a power in W/m^2/Hz otherwise.
        T_cmb : float, optional
            The CMB temperature used to convert a temperature fluctuation 
            into power fluctuation (if absolute is False). The default value 
            is taken from Fixsen et al. 2009.
